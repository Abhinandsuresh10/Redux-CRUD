...
import { useGetSingleUsersQuery, useUpdateUserRoleMutation } from "../slices/adminApiSlice"
...
const AddUserScreen = () => {
    ...
    const { data , isLoading, isError, error } = useGetSingleUsersQuery(userId);
    
    const user = data?.user;
   
    const [ updateProfile ] = useUpdateUserRoleMutation();
    ...
    if (isLoading) {
      ...
      if (isError) {
        return <p className="text-black">Error: {error?.data?.message || "Failed to load user data"}</p>;
      }
